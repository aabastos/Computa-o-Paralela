...
void compute_path(vector<Flight>& flights, string to, vector<Travel>& travels, unsigned long t_min, unsigned long t_max, Parameters parameters){
  ...
  while(travels.size() > 0){
    ...
    if(current_city.to == to){
      ...
      #pragma omp parallel for private(current_city) num_threads(4) schedule(dynamic, 10000)
      for(unsigned int i=0; i<flights.size(); i++){
	    Flight flight = flights[i];
	    if(flight.from == current_city.to && flight.take_off_time >= t_min && flight.land_time <= t_max && (flight.take_off_time > current_city.land_time) && flight.take_off_time - current_city.land_time <= parameters.max_layover_time && nerver_traveled_to(travel, flight.to)){
